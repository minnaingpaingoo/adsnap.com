...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 22, 2024 at 04:31 PM
...
--
DELIMITER $$
CREATE TRIGGER `ads_log` AFTER UPDATE ON `advertising` FOR EACH ROW insert into advertising_log(CategoryId, CustomerId,UploadDate, Status) values (NEW.CategoryId, NEW.CustomerId, NEW.UploadDate,NEW.Status)
$$
DELIMITER ;
...
CREATE DEFINER=`root`@`localhost` EVENT `delete_cancel_ads1` ON SCHEDULE EVERY 3 HOUR STARTS '2024-02-22 22:00:30' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM advertising_log
    WHERE Status=2 AND UploadDate < NOW() - INTERVAL 1 DAY$$

CREATE DEFINER=`root`@`localhost` EVENT `delete_not_confirm_ads1` ON SCHEDULE EVERY 15 MINUTE STARTS '2024-02-22 22:01:20' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM advertising_log
    WHERE Status=0 AND UploadTime < NOW() - INTERVAL 1 HOUR$$
